...
curl -v -s 'localhost:'$PORT'/'
curl -v -s 'localhost:'$PORT'/css?family=Roboto:300,400,400i,700,700i&display=swap'
curl -v -s -o/dev/null 'localhost:'$PORT'/npm/jquery@3.6.4/dist/jquery.min.js'
curl -v -s -o/dev/null 'localhost:'$PORT'/ajax/libs/jqueryui/1.13.3/jquery-ui.min.js'
curl -v -s 'localhost:'$PORT'/debug'
